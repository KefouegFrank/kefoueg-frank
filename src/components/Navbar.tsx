    ...
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-[900] transition-all duration-300 ${
          isScrolled
            ? "py-4 bg-black/80 backdrop-blur-lg border-b border-hud-cyan/10"
            : "py-6 bg-transparent"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          >
            <div className="relative w-10 h-10 flex items-center justify-center">
              ...
              <svg
                viewBox="0 0 100 100"
                className="w-full h-full fill-none stroke-hud-cyan stroke-[4px] drop-shadow-[0_0_8px_var(--hud-cyan-glow)]"
              >
                <path d="M50 5 L90 25 L90 75 L50 95 L10 75 L10 25 Z" />
                <path
                  d="M30 40 L50 40 L50 70 M50 55 L70 55"
          ...
          <div className="flex items-center gap-4 md:gap-8">
            ...
            {/* Tablet Burger - Centered (Only visible between md and lg) */}
            <div className="hidden md:flex lg:hidden flex-1 justify-center">
              <button
                className="w-10 h-10 flex flex-col items-center justify-center gap-1.5 transition-colors"
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              >
                <div className="w-6 h-[2px] bg-white" />
                <div className="w-4 h-[2px] bg-hud-cyan" />
                <div className="w-6 h-[2px] bg-white" />
              </button>
            </div>

            {/* Actions Container - Expand on Tablet to keep balance */}
              ...
              <div className="relative">
                ...
                >
                  <div className="relative w-4 h-4">
                    <svg
                      viewBox="0 0 24 24"
                      className="w-full h-full fill-none stroke-current stroke-1.5 opacity-60 group-hover:opacity-100 transition-opacity"
                    >
                    ...
                    {isLangOpen && (
                      <div className="absolute inset-0 bg-hud-cyan/20 blur-sm rounded-full animate-pulse" />
                    )}
                  </div>
                  <span className={isLangOpen ? "text-hud-cyan" : ""}>
                    {currentLang}
                  </span>
                  <svg
                    viewBox="0 0 24 24"
                ...
                >
                  ...
                  {languages.map((lang) => (
                    ...
                    >
                      {lang.label}
                      {currentLang === lang.code && (
                        <div className="w-1 h-1 rounded-full bg-hud-cyan shadow-[0_0_5px_var(--hud-cyan)]" />
                      )}
                    </button>
                  ))}
            ...
            >
              <div
                className={`w-6 h-[2px] ${isMobileMenuOpen ? "bg-white" : "bg-white"} transition-all ${isMobileMenuOpen ? "rotate-45 translate-y-[8px]" : ""}`}
              />
              <div
                className={`w-4 h-[2px] bg-hud-cyan transition-opacity ${isMobileMenuOpen ? "opacity-0" : ""}`}
              />
              <div
                className={`w-6 h-[2px] ${isMobileMenuOpen ? "bg-white" : "bg-white"} transition-all ${isMobileMenuOpen ? "-rotate-45 -translate-y-[8px]" : ""}`}
              />
      ...
      >
        ...
        >
          <div className="w-6 h-[2px] bg-white rotate-45 translate-y-[4px]" />
          <div className="w-6 h-[2px] bg-white -rotate-45 -translate-y-[4px]" />
        </button>
